...
class HFConvertConfig:
    ...
    openvla_model_path_or_id: Union[str, Path] = (                      # Path to Pretrained VLA (on disk or HF Hub)
        "/home/pl217/fantasingleft"
    )
    output_hf_model_local_path: Path = Path(                            # Path to Local Path to save HF model
        "/home/pl217/fantasingleft-hf"
    )
    output_hf_model_hub_path: str = ""                                  # Made optional by setting empty default
    
    # Made token optional with None default
    hf_token: Union[str, Path, None] = None                            

    def __post_init__(self) -> None:
        # Only try to read token if path is provided
        if isinstance(self.hf_token, Path) and self.hf_token.exists():
...
def convert_openvla_weights_to_hf(cfg: HFConvertConfig) -> None:
    ...
    # Get `config.json`, 'dataset_statistics.json' and `checkpoint_pt`
    if os.path.isdir(cfg.openvla_model_path_or_id):
        ...
        # Changed to look for safetensors files instead of PyTorch checkpoint
        model_files = list(run_dir.glob("model-*.safetensors"))
        assert len(model_files) > 0, f"No model files found in {run_dir}"
    ...
    with open(config_json, "r") as f:
        ...
        print("[DEBUG] Config file contents:", json.dumps(config_data, indent=2))
        
        # Try to get VLA config, falling back to top-level if "vla" key doesn't exist
        vla_cfg = config_data.get("vla", config_data)

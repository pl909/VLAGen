...
env_name=GraspSingleOpenedFantaCanInScene-v0
...
do CUDA_VISIBLE_DEVICES=5,6 python simpler_env/main_inference.py --policy-model openvla_generate_data --ckpt-path ${ckpt_path} \
  --robot google_robot_static \
  --control-freq 3 --sim-freq 513 --max-episode-steps 80 \
  --env-name ${env_name} --scene-name ${scene_name} \
  --rgb-overlay-path ${rgb_overlay_path} \
  --data-dir Fanta_Single \
